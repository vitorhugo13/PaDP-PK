...
int main(void) 
{
   ...
   for(i=0;i<N;i++)
    ...
    #pragma omp parallel for shared(A,B,C,i) private(sum,j,k) //used in scenario 1 and 2 (comment when trying scneario 0)
    for (i = 0; i < N; i++) 
    {
      for (j = 0; j < N; j++)
      {
        sum = 0;
      //   #pragma omp parallel for reduction(+:sum)     //used in scenario 2 (comment when trying scenario 0 and 1)                
        for (k = 0; k < N; k++)
        {
            sum += A[i][k] * B[k][j];
        }
