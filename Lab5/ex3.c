    ...
    *input_number = 0;
    ...
    while(*input_number >= 0){
        ...
        if(rank != 0){ //receiving number from rank before 
            MPI_Recv(input_number,1,MPI_INT,(rank - 1),MPI_ANY_TAG,MPI_COMM_WORLD,&status); 
        ...
        if(rank + 1 == size){ //means we need to send the value to the start of the ring
            MPI_Send(input_number,1,MPI_INT,0,tag,MPI_COMM_WORLD);
        }
        else{
            MPI_Send(input_number,1,MPI_INT,rank + 1,tag,MPI_COMM_WORLD);
        }
